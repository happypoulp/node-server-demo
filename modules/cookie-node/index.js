...
var mutateHttp = function(http){
  ...
  http.ServerResponse.prototype.writeHead = function () {
    ...
    if (!headers)
    {
        // No header arg - append one to args list
        args.push(headersArray);
    }
    else if(typeof(headers) == 'object' && headers.constructor.toString().indexOf('Array') == -1)
    {
        // Remove current header from arg list
        args = args.slice(0, 1);
        // Get all key / values from old header...
        for (key in headers)
        {
            // ...and add them to new header built as an array...
            headersArray.push([key, headers[key]]);
        }
        // ...then push the new header array to args list.
        args.push(headersArray);
    }
    else
    {
        // header is already an array, use it for coming cookies appending
    ...
    {
        //...append it to new header array.
        headersArray.push([COOKIE_KEY, cookies[i]]);
    }
